...
class Nerf:
    ...
    def from_dict(obj: Any) -> "Nerf":
        ...
        rendered_video_path = from_union(
            [from_str, from_none], obj.get("rendered_video_path")
        )
        return Nerf(model_file_path, rendered_video_path)

    def to_dict(self) -> dict:
        result: dict = {}
        result["model_file_path"] = from_union(
            [from_str, from_none], self.model_file_path
        )
        result["rendered_video_path"] = from_union(
            [from_str, from_none], self.rendered_video_path
        )
...
class Sfm:
    ...
    def from_dict(obj: Any) -> "Sfm":
        ...
        frames = from_union(
            [lambda x: from_list(Frame.from_dict, x), from_none], obj.get("frames")
        )
        return Sfm(intrinsic_matrix, frames)

    def to_dict(self) -> dict:
        result: dict = {}
        result["intrinsic_matrix"] = from_union(
            [lambda x: from_list(lambda x: from_list(from_float, x), x), from_none],
            self.intrinsic_matrix.tolist(),
        )
        result["frames"] = from_union(
            [lambda x: from_list(lambda x: to_class(Frame, x), x), from_none],
            self.frames,
        )
...
class Scene:
    ...
    def to_dict(self) -> dict:
        ...
        result["video"] = from_union(
            [lambda x: to_class(Video, x), from_none], self.video
        )
...
class SceneManager:
    ...
    def get_scene(self, _id: str) -> Optional[Scene]:
        ...
        return scene_from_dict(doc) if doc else None

    def get_video(self, _id: str) -> Optional[Video]:
        key = {"_id": _id}
        doc = self.collection.find_one(key)

        return Video.from_dict(doc.get("video")) if doc and "video" in doc else None

    def get_sfm(self, _id: str) -> Optional[Sfm]:
        key = {"_id": _id}
        doc = self.collection.find_one(key)

        return Sfm.from_dict(doc["sfm"]) if doc and "sfm" in doc else None

    def get_nerf(self, _id: str) -> Nerf:
        key = {"_id": _id}
        doc = self.collection.find_one(key)

        return Nerf.from_dict(doc["nerf"]) if doc and "nerf" in doc else None
