...
class RabbitMQService:
    ...
    def __init__(self):
        ...
        parameters = pika.ConnectionParameters(
            "localhost", 5672, "/", credentials, heartbeat=300
        )
        # Change this ->
    ...
    def publish_sfm_job(self, id: str, vid: Video):
        ...
        """
        job = {"id": id, "file_path": self.to_url(vid.file_path)}
    ...
    def publish_nerf_job(self, id: str, vid: Video, sfm: Sfm):
        ...
        """
        job = {"id": id, "vid_width": vid.width, "vid_height": vid.height}
...
def digest_finished_sfms(scene_manager: SceneManager):
    ...
    def process_sfm_job(ch, method, properties, body):
        ...
        # store png
        # is the iterator a frame?
        for idx, frame in enumerate(sfm_data["frames"]):
            # TODO: This code trusts the file extensions from the worker
            # TODO: handle files not found
            url = frame["file_path"]
            img = requests.get(url)
            url_path = urlparse(frame["file_path"]).path
            ...
            sfm_data["frames"][idx]["file_path"] = file_path
