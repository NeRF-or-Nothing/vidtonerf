...
def extract_position_data(infile, outfile, debug=False) -> Status:
    # Open input file
    try:
        file = open(infile, 'r')
    ...
    for line in lines:
        ...
        if words[-1].endswith(".JPG\n"):
            ...
            if debug:
                print(imageName + "- QW:" + qw + ", QX:" + qx + ", QY:" + qy +
...
if __name__ == '__main__':
    ...
    if status == Status.SUCCESS:
        print(
            f"Data successfully extracted from {infile} and placed in {outfile}")
    elif status == Status.UNKNOWN:
        print(
            f"ERROR: Unspecified error extracting from {infile} or placing in {outfile}")
